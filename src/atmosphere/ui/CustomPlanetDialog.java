...
public class CustomPlanetDialog extends PlanetDialog
{
    ...
    public CustomPlanetDialog()
    {
        ...
        addListener(new InputListener()
        {
            ...
            {
                if (transitions.IsRunning()) return false;

                if (event.targetActor == CustomPlanetDialog.this && (key == KeyCode.escape || key == KeyCode.back || key == Core.keybinds.get(Binding.planet_map).key))
                {
        ...
        addListener(new InputListener()
        {
            ...
            {
                if (transitions.IsRunning()) return true;

                if (event.targetActor == CustomPlanetDialog.this)
                {
    ...
    public void setup()
    {
        ...
        }).visible(() -> mode != select && shouldRenderGUI);
        ...
        stack(
                new Element()
                {
                    ...
                    {
                        if (scene.getDialog() == CustomPlanetDialog.this && !scene.hit(input.mouseX(), input.mouseY(), true).isDescendantOf(e -> e instanceof ScrollPane))
                        {
                            scene.setScrollFocus(CustomPlanetDialog.this);
                        }
    ...
    public void rebuildExpand()
    {
        Table c = expandTable;
        c.clear();
        c.visible(() -> !(graphics.isPortrait() && mobile) && shouldRenderGUI);
    ...
    public void displayItems(Table c, float scl, ObjectMap<Item, ExportStat> stats, String name, Cons<Table> builder)
    {
    ...
    private void LoadSector(Sector sector)
    {
        ...
        { //TODO make sure it doesn't have a base already.
    ...
    {
        set.set(initial).slerp(target, t);
    }

    private float TransformTransition(float t)
    {
        return Interp.sineOut.apply(t);
    }
    ...
    private void GetTargetVector(Sector sector, Vec3 set)
    {
        if (sector == null || sector.tile == PlanetGrid.Ptile.empty)
        {
            set.set(Vec3.Z);
            return;
        }

        Vars.ui.planet.state.planet = sector.planet;

        set.set(sector.tile.v).rotate(Vec3.Y, -sector.planet.getRotation());
    }
}